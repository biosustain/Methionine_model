---
title: "Maud: Bayesian regression modelling facilitates quantitative modelling of cell metabolism"
...
Our application Maud provides the first Bayesian kinetic model to combine
...
@fig-posterior, which shows two marginally independent parameters. This kind of
marginal independence does not mean that the parameters are independent in the
...
for two pairwise correlated parameters, namely $K_m^{AHC1,hcys-L}$ and
$K_m^{AHC1_ahcys}$. (**Bottom right**) Pairwise marginal posterior distribution
for two pairwise-uncorrelated parameters, namely $K_m^{MAT3,atp}$ and $K_{cat}^{MAT3}$.
