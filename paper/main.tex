...
\hypersetup{
  pdftitle={Maud: Bayesian regression modelling facilitates quantitative modelling of cell metabolism},
...
\title[]{Maud: Bayesian regression modelling facilitates quantitative
...
\end{abstract}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, enhanced, interior hidden, breakable, sharp corners, borderline west={3pt}{0pt}{shadecolor}, boxrule=0pt]}{\end{tcolorbox}}\fi
...
parameters. This kind of marginal independence does not mean that the
parameters are independent in the full joint posterior distribution.
...
pairwise correlated parameters, namely \(K_m^{AHC1,hcys-L}\) and
\(K_m^{AHC1_ahcys}\). (\textbf{Bottom right}) Pairwise marginal
posterior distribution for two pairwise-uncorrelated parameters, namely
