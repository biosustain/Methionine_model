...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, frame hidden, boxrule=0pt, interior hidden, sharp corners, breakable, enhanced]}{\end{tcolorbox}}\fi
...
\caption{\label{tbl-params}Parameters of Maud's statistical
model}\tabularnewline
\toprule\noalign{}
...
\eqref{eq-saturation}, which is taken from Liebermeister, Uhlendorf, and
Klipp (2010) and Noor et al. (2013). Additionally, this term captures
competitive inhibition: as competitive inhibitor concentration
increases, the saturation denominator increases, effectively decreasing
the saturation of the substrate on the total enzyme pool. Conversely, as
the substrate concentration increases this term approaches 1.
...
Monod, Wyman, and Changeux (1965), Changeux (2013), Popova and Sel'kov
(1975) and Popova and Sel'kov (1979).
...
Starting with the model in Saa and Nielsen (2016), we extracted values
for enzyme concentrations, boundary conditions and fluxes. We used these
values to generate MCMC samples using Maud using the priors specified in
section Section~\ref{sec-methionine-priors}. When this was finished, we
selected one sample with relatively high log probability to use as a
ground truth in our case study. These parameter values are shown below
in table Table~\ref{tbl-case-study-params}. We manually inspected the
parameter values to screen for any obviously implausible values; we did
...
\caption{\label{tbl-case-study-params}Parameter specification, marginal
prior distributions and true parameter values used in our case
study.}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
parameter name
...
derived from eQuilibrator (Beber et al. 2021).
...
\leavevmode\vadjust pre{\hypertarget{ref-beberEQuilibratorPlatformEstimation2021}{}}%
Beber, Moritz E., Mattia G. Gollub, Dana Mozaffari, Kevin M. Shebek, and
Elad Noor. 2021. {``{eQuilibrator} 3.0 -- a Platform for the Estimation
of Thermodynamic Constants.''} \emph{arXiv:2103.00621 {[}q-Bio{]}},
February. \url{http://arxiv.org/abs/2103.00621}.
...
Erika Dudkin, Janine Egert, Fabian Frö hlich, et al. 2021.
