...
PLOT_OUTPUT = Path.cwd() / "paper" / "figures" / "missing.png"
...
def plot_residual_state_variables(residual_df, ax):
    for model, direction in zip(
        ...
        df = residual_df.loc[lambda df: df["model"] == model]
        x = np.arange(len(df)) + 0.1 * direction
        ax.vlines(
            x=x,
            ymin=df[0.05],
            ymax=df[0.95],
...
def plot_figure():
    ...
    csvs_methionine_missing = [
        MAUD_OUTPUT_MISSING_AHCYS / "samples" / file
        for file in os.listdir(MAUD_OUTPUT_MISSING_AHCYS / "samples")
        if ".csv" in file
    ]
    infd_methionine_missing = get_idata(
        csvs_methionine_missing, mi=mi_missing, mode="train"
    )
    ...
    flux_methionine = (
        infd_methionine.posterior.flux_train.to_dataframe()
        .dropna()
        .droplevel(1)
        .reset_index()
    )
    flux_methionine_missing = (
        infd_methionine_missing.posterior.flux_train.to_dataframe()
        .dropna()
        .droplevel(1)
        .reset_index()
    )
    ...
    for _, row in flux_methionine.iterrows():
        ...
        if id in flux_meas_df.index:
            lp_flux_methionine.append(
                ll_normal(
                    row["flux_train"],
                    flux_meas_df.loc[id]["flux"],
                    flux_meas_df.loc[id]["uncertainty"],
                )
            )
    ...
    for _, row in flux_methionine_missing.iterrows():
        id = row["experiments"] + row["reactions"]
        if id in flux_meas_df.index:
            lp_flux_methionine_missing.append(
                ll_normal(
                    row["flux_train"],
                    flux_meas_df.loc[id]["flux"],
                    flux_meas_df.loc[id]["uncertainty"],
    ...
    conc_methionine = (
        infd_methionine.posterior.conc_train.sel(mics=balanced_mics)
        .to_dataframe()
        .dropna()
        .droplevel(1)
        .reset_index()
    ...
    conc_methionine_missing = (
        infd_methionine_missing.posterior.conc_train.sel(mics=balanced_mics)
        .to_dataframe()
        .dropna()
        .droplevel(1)
        .reset_index()
    ...
    met_AHC1_ahcys = (
        infd_methionine.posterior.km.sel(kms=["AHC1_ahcys_c"])
        .to_dataframe()
        .reset_index()
    )
    met_AHC1_ahcys["model"] = "complete measurement set"
    met_missing_ahcys_AHC1_ahcys = (
        infd_methionine_missing.posterior.km.sel(kms=["AHC1_ahcys_c"])
        .to_dataframe()
        .reset_index()
    )
    met_missing_ahcys_AHC1_ahcys["model"] = "missing ahcys measurement"
    AHC1_ahcys_df = pd.concat(
        [met_AHC1_ahcys, met_missing_ahcys_AHC1_ahcys]
    ).reset_index()
    ...
    for i, row in conc_methionine.iterrows():
        ...
        conc_methionine.loc[i, "log_residual"] = np.log(row["conc_train"]) - np.log(
            conc_meas_df.loc[id]["conc"]
        )
    conc_methionine = conc_methionine.loc[
        conc_methionine["experiments"] != "validation"
    ]

    for i, row in conc_methionine_missing.iterrows():
        id = row["experiments"] + row["mics"]
        conc_methionine_missing.loc[i, "log_residual"] = np.log(
            row["conc_train"]
        ) - np.log(conc_meas_df.loc[id]["conc"])
    conc_methionine_missing = conc_methionine_missing.loc[
        conc_methionine_missing["experiments"] != "validation"
    ]

    conc_validation = pd.concat([conc_methionine, conc_methionine_missing], axis=0)
    conc_validation = (
        conc_validation.groupby(["experiments", "mics", "model"])
        .quantile([0.05, 0.95])
        .reset_index()
        .pivot(
            index=["mics", "experiments", "model"],
            columns="level_3",
            values="log_residual",
        )
        .reset_index()
    ...
    samples_legend = mpatches.Patch(
        color=colour_scheme["complete measurement set"],
        label="complete measurement set",
    )
    measurements_legend = mpatches.Patch(
        color=colour_scheme["missing ahcys measurement"],
        label="missing ahcys measurement",
    )
    true_value_legend = Line2D([0], [0], color="r", lw=3, label="true value")

    fig.legend(
        handles=[samples_legend, measurements_legend, true_value_legend],
        loc="upper center",
        ncol=3,
        borderaxespad=0.0,
        fontsize=12,
