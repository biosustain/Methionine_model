...
def plot_lp_testing(axes, lp):
    color_map = {"posterior": "#001219", "laplace_approximation": "#005F73"}
    sns.histplot(
        data=lp,
        x="lp",
        hue="sample",
...
def lpdf_normal(flux, meas, scale):
    ...
    return np.log(
        (1.0 / (scale * np.sqrt(np.pi * 2.0)))
        * np.exp(-0.5 * ((flux - meas) / scale) ** 2.0)
    )
...
def plot_llik_flux_validation(df, axes):
    ...
    for rxn in df.reaction.unique():
        for exp in df.experiment.unique():
            ...
            count += 1
    axes[1, 0].set_ylabel("Log likelihood (validation fluxes)")
...
def plot_llik_flux_training(log_lik_df, axes):
    ...
    for id, sample in log_lik_df.loc[
        log_lik_df["sample"] == "laplace_approximation"
    ].iterrows():
        axes[0, 1].vlines(
            x=sample["id"] - 0.2,
            ymin=sample["lower_5"],
    ...
    axes[0, 1].set_ylabel("Log likelihood (training fluxes)")
...
def plot_figure():
    ...
    csvs_training = [
        MAUD_OUTPUT_TRAINING / "samples" / file
        for file in os.listdir(MAUD_OUTPUT_TRAINING / "samples")
        if ".csv" in file
    ]
    csvs_validation = [
        MAUD_OUTPUT_TRAINING / "validation" / file
        for file in os.listdir(MAUD_OUTPUT_TRAINING / "validation")
        if ".csv" in file
    ]
    csvs_laplace_training = [
        MAUD_OUTPUT_LAPLACE_TRAINING / "samples" / file
        for file in os.listdir(MAUD_OUTPUT_LAPLACE_TRAINING / "samples")
        if ".csv" in file
    ]
    csvs_laplace_validation = [
        MAUD_OUTPUT_LAPLACE_TRAINING / "validation" / file
        for file in os.listdir(MAUD_OUTPUT_LAPLACE_TRAINING / "validation")
        if ".csv" in file
    ]
    ...
    llik_flux = [
        {
            "sample": sample,
            "id": flux,
            "lower_5": idata.posterior.llik_flux_train.sel(llik_flux_train_dim_0=[flux])
            .to_series()
            .quantile(0.05),
            "upper_95": idata.posterior.llik_flux_train.sel(
                llik_flux_train_dim_0=[flux]
            )
            .to_series()
            .quantile(0.95),
        }
    ...
    flux_interval = [
        {
            ...
            "reaction": rxn,
            "experiment": exp,
            "lower_5_lp": idata.posterior.flux_test.sel(
                reactions=[rxn], experiments=[exp]
            )
            .to_series()
            .apply(
                lambda x: lpdf_normal(
                    x, meas_df.loc[exp + rxn].flux, meas_df.loc[exp + rxn].uncertainty
                )
            )
            .quantile(0.05),
            "upper_95_lp": idata.posterior.flux_test.sel(
                reactions=[rxn], experiments=[exp]
            )
            .to_series()
            .apply(
    ...
    par_2 = "r1"
    km_posterior = (
        idata_train.posterior.km.sel(kms=[par_1])
        .to_dataframe()
        .unstack()
        .km.droplevel(1)
        .reset_index()
    ...
    kcat_posterior = (
        idata_train.posterior.kcat.sel(enzymes=[par_2])
        .to_dataframe()
        .unstack()
        .kcat.droplevel(1)
        .reset_index()
    ...
    km_laplace = (
        idata_laplace_train.posterior.km.sel(kms=[par_1])
        .to_dataframe()
        .unstack()
        .km.droplevel(1)
        .reset_index()
    ...
    kcat_laplace = (
        idata_laplace_train.posterior.kcat.sel(enzymes=[par_2])
        .to_dataframe()
        .unstack()
        .kcat.droplevel(1)
        .reset_index()
    ...
    true_posterior_legend = mpatches.Patch(
        color=colour_scheme["posterior"], alpha=0.7, label="posterior"
    )
    laplace_legend = mpatches.Patch(
        color=colour_scheme["laplace"], alpha=0.7, label="laplace_approximation"
    )
    fig.legend(
        handles=[true_posterior_legend, laplace_legend],
        loc="upper center",
        ncol=2,
